...
c	      write(*,*)B_field_z(iz,ir),B_field_r(iz,ir)
          ENDDO
        ENDDO
        CLOSE (unit=1)
      ELSE
...
C      write(*,'(a11,3e12.3)') 'GUFLD: x = ',x_
     ...
     >    (j+1 .GT. nr) .OR. (j .LT. 1)) THEN                
      ...
      ELSE                                                                     
        ...
        B(3) = (ar*(a1-a0)+a0)
c	write(*,*)i,j,B(3),B_field_z(i+1,j+1),B_field_z(i,j+1),B_field_z(i+1,j),B_field_z(i,j)
        IF (r .gt. 0.) THEN
      ...
      Bout(2) = -B_(1)
      Bout(3) =  B_(3)
c	if(abs(z).lt.6.and.abs(r).lt.6)write(*,*)Bout,i,j+1,B_field_z(i+1,j),B_field_z(i,j),B_stheta,B_ctheta,B(3)
